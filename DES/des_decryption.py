from functions import hex_to_string, string_to_hex
...
class DES_DECRYPTION:
    def __init__(self,cipher_text,key):
        ...
        self.ciphertext = h_x(cipher_text)             
        ...
        self.expansion_permutation_table = [32,1,2,3,4,5,
                                            ...
                                            24,25,26,27,28,29,
                                            28,29,30,31,32,1]
        self.init_string = permutate(self.ciphertext,self.initial_permutation,len(self.ciphertext)) 
        self.right_bits = self.init_string[32:64]
        self.left_bits = self.init_string[:32]
        self.keys =  GET_DES_ENCRYPTION_KEY(key).get_decryption_round_key()
    ...
    def decrypt(self): 
        ...
        plaintext = permutate(round16_ciphertext,inverse_perm,len(round16_ciphertext))                               
        print(f"PLAINTEXT : {hex_to_string(binary_to_hex(plaintext))}")
        return binary_to_hex(plaintext)
my_des = DES_DECRYPTION('ec27a2005b66ad16','muchesa')
print(my_des.decrypt())
